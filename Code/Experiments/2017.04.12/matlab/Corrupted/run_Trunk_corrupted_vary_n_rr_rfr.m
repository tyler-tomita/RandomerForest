...
for j = 1:length(ps)
    ...
    Xtest = dlmread(sprintf('/scratch/groups/jvogels3/tyler/R/Data/Trunk/dat/Corrupted/Test/Trunk_test_set_p%d.dat',p));
    Ytest = cellstr(num2str(Xtest(:,end)));
    Xtest(:,end) = [];
    ClassPosteriors = dlmread(sprintf('/scratch/groups/jvogels3/tyler/R/Data/Trunk/dat/Corrupted/Test/Trunk_test_set_posteriors_p%d.dat',p));
        ...
        for c = 1:length(Classifiers)
            ...
            if strcmp(Params{i,j}.(Classifiers{c}).ForestMethod,'rerf')
                if strcmp(Params{i,j}.(Classifiers{c}).RandomMatrix,'frc')
                    ...
                    TreeStrength{i,j}.(Classifiers{c}) = NaN(ntrials,length(Params{i,j}.(Classifiers{c}).d)*length(Params{i,j}.(Classifiers{c}).L));
                    ...
                    TreeStrength{i,j}.(Classifiers{c}) = NaN(ntrials,length(Params{i,j}.(Classifiers{c}).d)*length(Params{i,j}.(Classifiers{c}).rho));
                ...
                TreeStrength{i,j}.(Classifiers{c}) = NaN(ntrials,length(Params{i,j}.(Classifiers{c}).d));
            ...
            for trial = 1:ntrials
                ...
                Xtrain = dlmread(sprintf('/scratch/groups/jvogels3/tyler/R/Data/Trunk/dat/Corrupted/Train/Trunk_train_set_n%d_p%d_trial%d.dat',ns{j}(i),p,trial));
            ...
            fprintf('%s complete\n',Classifiers{c})
            save([rerfPath 'RandomerForest/Results/2017.04.12/Trunk/Trunk_corrupted_vary_n_' Classifiers{c} '.mat'],'ps',...
