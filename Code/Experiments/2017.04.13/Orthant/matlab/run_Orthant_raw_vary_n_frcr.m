...
for j = 1:length(ps)
    ...
    Xtest = dlmread(sprintf('/scratch/groups/jvogels3/tyler/R/Data/Orthant/dat/Raw/Test/Orthant_raw_test_set_p%d.dat',p));
    Ytest = cellstr(num2str(Xtest(:,end)));
    Xtest(:,end) = [];
    ClassPosteriors = dlmread(sprintf('/scratch/groups/jvogels3/tyler/R/Data/Orthant/dat/Orthant_test_set_posteriors_p%d.dat',p));
    ...
    for i = 1:length(ns{j})
        ...
        for c = 1:length(Classifiers)
            ...
            fprintf('%s complete\n',Classifiers{c})
            save([rerfPath 'RandomerForest/Results/2017.04.13/Orthant/Orthant_raw_vary_n_' Classifiers{c} '.mat'],'ps',...
