% Test to compare randomization of sparsity (RerF) vs fixing sparsity (F-RC)
...
% for j = 1:length(ps)
    ...
    Xtest = dlmread(sprintf('/Users/tyler/R/Data/Sparse_parity/dat/Raw/Test/Sparse_parity_test_set_p%d.dat',p));
    Ytest = cellstr(num2str(Xtest(:,end)));
    Xtest(:,end) = [];
    ClassPosteriors = dlmread(sprintf('/Users/tyler/R/Data/Sparse_parity/dat/Raw/Test/Sparse_parity_test_set_posteriors_p%d.dat',p));
...
%                 Params{i,j}.(Classifiers{c}).rho = (1:min(p,3))/p;
                Params{i,j}.(Classifiers{c}).rho = 1/p;
            elseif strcmp(Classifiers{c},'rerfb') || strcmp(Classifiers{c},'rerfbr')
                Params{i,j}.(Classifiers{c}).ForestMethod = 'rerf';
                Params{i,j}.(Classifiers{c}).RandomMatrix = 'uniform-nnzs-binary';
                Params{i,j}.(Classifiers{c}).d = mtrys;
                Params{i,j}.(Classifiers{c}).nmix = 1:min(p,5);
                Params{i,j}.(Classifiers{c}).rho = 1/p;
            elseif strcmp(Classifiers{c},'rerfc') || strcmp(Classifiers{c},'rerfcr')
                Params{i,j}.(Classifiers{c}).ForestMethod = 'rerf';
                Params{i,j}.(Classifiers{c}).RandomMatrix = 'uniform-nnzs-continuous';
                Params{i,j}.(Classifiers{c}).d = mtrys;
                Params{i,j}.(Classifiers{c}).nmix = 1:min(p,5);
                Params{i,j}.(Classifiers{c}).rho = 1/p;        
            elseif strcmp(Classifiers{c},'frc') || strcmp(Classifiers{c},'frcr')
                ...
                Params{i,j}.(Classifiers{c}).d = mtrys;
                Params{i,j}.(Classifiers{c}).L = 2:min(p,5);    
            ...
            for trial = 1:ntrials
                ...
                Xtrain = dlmread(sprintf('/Users/tyler/R/Data/Sparse_parity/dat/Raw/Train/Sparse_parity_train_set_n%d_p%d_trial%d.dat',ns{j}(i),p,trial));
                ...
                for k = 1:length(Forest)
                    ...
                    if ~strcmp(Forest{k}.Rescale,'off')
                        Scores = rerf_classprob(Forest{k},Xtest,'individual',false,Xtrain);
                    else
                        Scores = rerf_classprob(Forest{k},Xtest,'individual',false);
                    end
                    ...
                    if ~strcmp(Forest{k}.Rescale,'off')
                        Scores = rerf_classprob(Forest{k},Xtest,'last',false,Xtrain);
                    else
                        Scores = rerf_classprob(Forest{k},Xtest,'last',false);
                    end
            ...
            fprintf('%s complete\n',Classifiers{c})
            save([rerfPath 'RandomerForest/Results/2017.05.22/Sparse_parity_raw_vary_n_' Classifiers{c} '.2017.05.22.mat'],'ps',...
