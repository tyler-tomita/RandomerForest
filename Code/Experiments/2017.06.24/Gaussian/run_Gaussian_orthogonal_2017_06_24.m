%% Evaluate on Gaussian orthogonal binary classification problem
...
SE= cell(length(ns),length(ps));
VE = cell(length(ns),length(ps));
...
for j = 1:length(ps)
    ...
    Xtest = dlmread(sprintf('~/R/Data/Gaussian/dat/Test/Gaussian_orthogonal_test_set_p%d.dat',p));
    Ytest = cellstr(num2str(Xtest(:,end)));
    Xtest(:,end) = [];
    ClassPosteriors = dlmread(sprintf('~/R/Data/Gaussian/dat/Test/Gaussian_orthogonal_test_set_posteriors_p%d.dat',p));
        ...
        for c = 1:length(Classifiers)
            ...
            if strcmp(Params{i,j}.(cl).ForestMethod,'rerf')
                if strcmp(Params{i,j}.(cl).RandomMatrix,'frc')
                    ...
                    SE{i,j}.(cl) = NaN(1,length(Params{i,j}.(cl).d)*length(Params{i,j}.(cl).L));
                    VE{i,j}.(cl) = NaN(1,length(Params{i,j}.(cl).d)*length(Params{i,j}.(cl).L));                    
                    ...
                    SE{i,j}.(cl) = NaN(1,length(Params{i,j}.(cl).d)*length(Params{i,j}.(cl).rho));
                    VE{i,j}.(cl) = NaN(1,length(Params{i,j}.(cl).d)*length(Params{i,j}.(cl).rho));                    
                ...
                SE{i,j}.(cl) = NaN(1,length(Params{i,j}.(cl).d));
                VE{i,j}.(cl) = NaN(1,length(Params{i,j}.(cl).d));                
            ...
            for trial = 1:ntrials
                ...
                Xtrain = dlmread(sprintf('~/R/Data/Gaussian/dat/Train/Gaussian_orthogonal_train_set_n%d_p%d_trial%d.dat',ns(i),p,trial));
            ...
            [Bias{i,j}.(cl),Variance{i,j}.(cl),SE{i,j}.(cl),VE{i,j}.(cl)] = bias_variance(TP,ClassPosteriors);
            fprintf('%s complete\n',cl)
            save([rerfPath 'RandomerForest/Results/2017.06.24/Gaussian/Gaussian_orthogonal_2017_06_24.mat'],'ps',...
