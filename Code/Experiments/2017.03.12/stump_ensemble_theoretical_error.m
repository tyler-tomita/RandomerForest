...
axWidth = 1.5;
axHeight = 1.5;
axLeft = repmat([FontSize*7,FontSize*12+axWidth,FontSize*17+2*axWidth],1,3);
...
ylabel('x_2')
zlabel('f_X(x)')
title({'\bf{Distribution}';'(oblique view)'})
text(-0.50,0.5,'\bf{\theta = 0°}','FontSize',14,'FontWeight','bold','Units',...
...
j = 2;
ax(j) = axes;
heat_map(X1,X2,f_X,parula,min(f_X(:)),max(f_X(:)),false);
xlabel('x_1')
ylabel('x_2')
title({'\bf{Distribution}';'(overhead view)'})
ax(j).LineWidth = LineWidth;
ax(j).FontUnits = 'inches';
ax(j).FontSize = FontSize;
ax(j).Units = 'inches';
...
ylabel('x_2')
zlabel('f_X(x)')
text(-0.50,0.5,'\bf{\theta = 22.5°}','FontSize',14,'FontWeight','bold','Units',...
'normalized','HorizontalAlignment','center','VerticalAlignment'...
,'bottom','Rotation',90)
ax(j).LineWidth = LineWidth;
ax(j).FontUnits = 'inches';
...
ylabel('x_2')
zlabel('f_X(x)')
text(-0.50,0.5,'\bf{\theta = 45°}','FontSize',14,'FontWeight','bold','Units',...
'normalized','HorizontalAlignment','center','VerticalAlignment'...
,'bottom','Rotation',90)
ax(j).LineWidth = LineWidth;
ax(j).FontUnits = 'inches';
...
[lh,objh] = legend('RF(depth = 1)','RerF(depth = 1)','Bayes');
lh.FontSize = 14;
lh.Units = 'inches';
