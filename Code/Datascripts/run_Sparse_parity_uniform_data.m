...
for i = 1:ndims
    ...
    Xtest(i).Untransformed = rand(ntest,p)*2 - 1;
    Ytest(i).Untransformed = cellstr(num2str(mod(sum(Xtest(i).Untransformed(:,1:p_prime)>0,2),2)));    
    for trial = 1:ntrials
        R = random_rotation(p);
...
save('~/Documents/MATLAB/Data/Sparse_parity_uniform_data.mat','Xtrain','Ytrain',...
