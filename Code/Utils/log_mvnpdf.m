...
%LOG_MVNPDF Multivariate normal log probability density function (pdf).
%   Y = LOG_MVNPDF(X) returns the log probability density of the multivariate normal
...
%   N-by-1 vector.
%
%   Y = LOG_MVNPDF(X,MU) returns the log density of the multivariate normal
...
%   match the size of X.
%
%   Y = LOG_MVNPDF(X,MU,SIGMA) returns the log density of the multivariate normal
...
%   of SIGMA, i.e., LOG_MVNPDF computes Y(I) using X(I,:) and SIGMA(:,:,I).
...
%
%   If X is a 1-by-D vector, LOG_MVNPDF replicates it to match the leading
...
if nargin<1
    error(message('log_mvnpdf:TooFewInputs'));
elseif ndims(X)~=2
    error(message('log_mvnpdf:InvalidData'));
end

% Get size of data.  Column vectors provisionally interpreted as multiple scalar data.
[n,d] = size(X);
if d<1
    error(message('log_mvnpdf:TooFewDimensions'));
end
...
elseif ndims(Mu) == 2
    ...
    if d2 ~= d % has to have same number of coords as X
        error(message('log_mvnpdf:ColSizeMismatch'));
    ...
    elseif n == 1 % data is a single row, rep it out to match mean
        ...
        error(message('log_mvnpdf:RowSizeMismatch'));
    end
    
else
    error(message('log_mvnpdf:BadMu'));
end
...
elseif ndims(Sigma) == 2
    ...
    if sz(1) ~= sz(2)
        error(message('log_mvnpdf:BadCovariance'));
    elseif ~isequal(sz, [d d])
        error(message('log_mvnpdf:CovSizeMismatch'));
    else
        if sigmaIsDiag
            if any(Sigma<=0)
                error(message('log_mvnpdf:BadDiagSigma'));
            end
            R = sqrt(Sigma);
            ...
            if err ~= 0
                error(message('log_mvnpdf:BadMatrixSigma'));
            end
...
elseif ndims(Sigma) == 3
    ...
    if sz(1) ~= sz(2)
        error(message('log_mvnpdf:BadCovarianceMultiple'));
    elseif (sz(1) ~= d) || (sz(2) ~= d) % Sigma is a stack of dxd matrices
        error(message('log_mvnpdf:CovSizeMismatchMultiple'));
    elseif sz(3) ~= n
        error(message('log_mvnpdf:CovSizeMismatchPages'));
    else
        if sigmaIsDiag
            if any(any(Sigma<=0))
                error(message('log_mvnpdf:BadDiagSigma'));
            ...
            for i = 1:n
                ...
                if err ~= 0
                    error(message('log_mvnpdf:BadMatrixSigmaMultiple'));
                end
                xRinv(i,:) = X0(i,:) / R;
