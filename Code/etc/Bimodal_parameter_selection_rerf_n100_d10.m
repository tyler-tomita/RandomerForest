...
mu0 = cat(2,-1*ones(1,d/2),ones(1,d/2));
mu1 = -mu0;
Mu = cat(1,mu0,mu1);
Sigma = ones(1,d);
...
save(sprintf('Bimodal_parameter_selection_rerf_n%d_d%d.mat',n,d),'err_rf','err_rerf')
...
xlabel('# trees')
ylabel('oob error')
title(sprintf('Bimodal (n=%d, d=%d, ntrials=%d)',n,d,ntrials))

save_fig(gcf,sprintf('Bimodal_parameter_selection_rerf_n%d_d%d',n,d))
...
save_fig(gcf,sprintf('Bimodal_parameter_selection_rerf_variance_n%d_d%d',n,d))
...
ylabel('oob error')
title(sprintf('Bimodal (n=%d, d=%d, ntrials=%d)',n,d,ntrials))
save_fig(gcf,sprintf('Bimodal_parameter_selection_rerf_errorbar_n%d_d%d',n,d))
