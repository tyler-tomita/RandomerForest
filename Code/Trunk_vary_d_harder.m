...
for trial = 1:ntrials
    ...
    for i = 1:length(dims)
        ...
        tic
        f2 = rpclassificationforest(ntrees,X,Y,'s',3,'nvartosample',nvartosample,'mdiff','off','sparsemethod','fast','NWorkers',NWorkers);
        tf2(trial,i) = toc;
        ...
        tic
        f3 = rpclassificationforest(ntrees,X,Y,'s',3,'nvartosample',nvartosample,'mdiff','on','sparsemethod','fast','NWorkers',NWorkers);
        tf3(trial,i) = toc;
        ...
        tic
        f4 = rpclassificationforest(ntrees,X,Y,'s',3,'nvartosample',nvartosample,'mdiff','on','sparsemethod','fast','Robust',true,'NWorkers',NWorkers);
        tf4(trial,i) = toc;
...
fname = sprintf('Trunk_harder_ooberror_vs_d_n%d_ntrees%d_ntrials%d',n,ntrees,ntrials);
save_fig(gcf,fname)
...
title('Trunk')
legend('RandomForest','TylerForest','TylerForest+','TylerForest+meandiff','Robust')
fname = sprintf('Trunk_harder_time_vs_d_n%d_ntrees%d_ntrials%d',n,ntrees,ntrials);
save_fig(gcf,fname)