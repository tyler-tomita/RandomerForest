...
axHeight = 1;
FontSize = 0.1;
axLeft = repmat([FontSize*5,FontSize*10+axWidth,FontSize*15+axWidth*2],1,6);
axBottom = [repmat(FontSize*23+axHeight*5,1,3),...
    repmat(FontSize*19+axHeight*4,1,3),repmat(FontSize*15+axHeight*3,1,3),...
...
for i = 1:length(dims)
    ...
    for j = 1:length(Metrics)
        ...
        elseif i == length(dims) && j == length(Metrics)
...
save_fig(gcf,[rerfPath 'RandomerForest/Figures/Sparse_parity_bias_variance'])