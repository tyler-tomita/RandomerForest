...
axBottom = [(FontSize*5+axHeight)*ones(1,2),...
    FontSize*2.5*ones(1,2)];
legWidth = axWidth;
legHeight = axHeight;
...
for i = 1:length(Transformations)
    ...
    for j = 1:length(Classifiers)
        errorbar(dims,mean(ErrorMatrix.(Classifiers{j}).(Transformations{i})),...
            ...
            'LineWidth',LineWidth.(Classifiers{j}),'Color',Colors.(Classifiers{j}),...
    ...
    if strcmp(Transformations{i},'Outlier')
        ylabel('\bf{Corrupted}')
        ylabel({'\bf{Corrupted}';'\rm{Error Rate}'})
    else
        ylabel({['\bf{' Transformations{i} '}'];'\rm{Error Rate}'})
    end
    
    ax(2*i-1).LineWidth = LineWidth.ax;
...
save_fig(gcf,[rerfPath 'RandomerForest/Figures/ROFLMAO_fig3_transformations_slide_version'],{'fig','pdf','png'})