...
%legend('rf (mtry = d^1^/^4)','rf (mtry = d^1^/^2)','rf (mtry = d^3^/^4)','rf (mtry = d)','rf_rot (mtry = d^1^/^4)','rf_rot (mtry = d^1^/^2)','rf_rot (mtry = d^3^/^4)','rf_rot (mtry = d)')
%legend('rf (mtry = 1)','rf (mtry = 2)','rf (mtry = 3)','rf rotate (mtry = 1)','rf rotate (mtry = 2)','rf rotate (mtry = 3)')
legend('rf (mtry = 1)','rf (mtry = 2)','rf rotate (mtry = 1)','rf rotate (mtry = 2)')
xlabel('# trees')
ylabel('oob error')
...
for mtry = mtrys
    plot(var_rf_rot(:,i-length(mtrys)),'Color',Colors(i,:))
    hold on
    i = i + 1;
end
...
for mtry = mtrys
    errorbar(1:ntrees,mean_err_rf_rot(:,i-length(mtrys)),sem_rf_rot(:,i-length(mtrys)),'Color',Colors(i,:))
    hold on
    i = i + 1;
end
