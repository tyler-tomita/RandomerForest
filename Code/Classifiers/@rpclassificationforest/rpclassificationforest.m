    ...
    methods
        ...
        function forest = growTrees(forest,X,Y,varargin)
            ...
            [Prior,Cost,Criterion,splitmin,minparent,minleaf,...
                ...
                AdjustmentFactors,DownsampleNode,MaxNodeSize,ImHeight,ImWidth,...
                ~,extra] = internal.stats.parseArgs(okargs,defaults,varargin{:});
            
            % Check structured random projection parameter
            if (strcmp(RandomMatrix,'image') || strcmp(RandomMatrix,'image-control')) ...
                    && ~isempty(ImHeight) && ~isempty(ImWidth)
                error('if doing structured image RerF, then must set image height and width');
            end
