...
require(devtools)
rmarkdown::render("plot_biau2008_consistency.Rmd")
...
runExperiment <- function(n.samples, seed = NULL) {
  ...
  for (i in 1:length(n.samples)) {
    ...
    # param.list <- list(p = p, d = d, sparsity = rho, prob = prob)
    mat.options <- list(p = p, d = d, random.matrix = 'binary', sparsity = rho, prob = prob)
    
    print(paste("Running forest with rerf", '(n =', n.samples[i], "samples)"))
  
    forest <- RerF(X, Y, trees = 500L, mat.options = mat.options, store.oob = F, seed = seed, num.cores = 1)
    Yhat <- Predict(X.test, forest, num.cores = 1)
    errors.rerf[i] <- sum(Y.test != Yhat) / length(Yhat)
    
    # param.list <- list(p = p, d = d, random.matrix = 'rf', rho = rho, prob = prob)
    mat.options <- list(p = p, d = d, random.matrix = 'rf', sparsity = rho, prob = prob)
...
err.plot <- ggplot(data = df, aes(x = n.samples, y = errors, color = Algorithm)) + 
  ...
  scale_color_manual(values=c("red", "black"), labels=c("RerF","RF")) +
...
ggsave('~/RandomerForest/Draft/PAMI/Figures/error_rate_consistency_tmt.pdf', plot = out, width = 9, height = 4)
```
