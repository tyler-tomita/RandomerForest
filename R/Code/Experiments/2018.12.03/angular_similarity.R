...
cos.sim <- function(u, v) {
    return(u%*%v/sqrt(sum(u^2)*sum(v^2)))
}
...
for (n.ind in 1:length(ns)) {
    ...
    for (trial in 1:num.trials) {
        ...
        theta.mag <- theta
        theta.mag[theta.mag > pi] <- abs(theta.mag[theta.mag > pi] - 2*pi)
        ...
        # tune RFDReg
        ds <- c(1L, 2L, 3L, 4L)
        Ysub <- Ysim[train.idx, train.idx]
        res <- tuneRFDReg(X[train.idx, ], c(diag(Ysub), Ysub[lower.tri(Ysub)]), Ysub, 'rf', ds)

        # retrain RFDReg with best hyperparameter
        start.time <- proc.time()

        forest <- RFDReg(X[train.idx, ], c(diag(Ysub), Ysub[lower.tri(Ysub)]),
                 bagging = 0.2, replacement = TRUE,
                 trees = 200L, min.parent = 2L, max.depth = 0,
    ...
    save(mae, map1, map10, map100, map500, train.time, file = '/Users/tyler/RandomerForest/R/Results/2018.12.03/angular_similarity.RData')
}
