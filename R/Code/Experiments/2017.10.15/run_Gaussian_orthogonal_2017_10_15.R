...
for (m in classifiers) {
  ...
  for (j in 1:length(ps)) {
    ...
    D <- read.table(paste0(dataPath, "Test/Gaussian_orthogonal_test_set_p", p, ".dat"), sep = ",")
    Xtest <- as.matrix(D[, 1:ncol(D)-1L])
    Ytest <- as.integer(D[, ncol(D)]) + 1L
    labels <- sort(unique(Ytest))
    ntest = nrow(Xtest)
    
    posteriors <- as.matrix(read.table(paste0(dataPath, "Test/Gaussian_orthogonal_test_set_posteriors_p", p, ".dat"), sep = ","))
    ...
    for (i in 1:length(ns)) {
      ...
      for (trial in 1:nTrials) {
        ...
        D <- read.table(paste0(dataPath, "Train/Gaussian_orthogonal_train_set_n", ntrain, "_p", p, "_trial", trial, ".dat"), sep = ",")
      ...
      save(testError, OOBError, OOBAUC, trainTime, OOBTime, testTime, treeStrength, treeCorr, numNodes, bestIdx, params,
           file = paste(rerfPath, "RandomerForest/R/Results/2017.10.15/Gaussian_orthogonal_2017_10_15.RData", sep = ""))
    }
  }
}
