...
for (m in classifiers) {
  ...
  testError[[dataSet]][[m]] <- matrix(as.double(rep(NA, nFolds*length(sparsity)*length(mtrys)*length(supervised)*length(prob))),
                                      nrow = nFolds, ncol = length(sparsity)*length(mtrys)*length(supervised)*length(prob))
  testAUC[[dataSet]][[m]] <- matrix(as.double(rep(NA, nFolds*length(sparsity)*length(mtrys)*length(supervised)*length(prob))),
                                      nrow = nFolds, ncol = length(sparsity)*length(mtrys)*length(supervised)*length(prob))
  OOBError[[dataSet]][[m]] <- matrix(as.double(rep(NA, nFolds*length(sparsity)*length(mtrys)*length(supervised)*length(prob))),
                                      nrow = nFolds, ncol = length(sparsity)*length(mtrys)*length(supervised)*length(prob))
  OOBAUC[[dataSet]][[m]] <- matrix(as.double(rep(NA, nFolds*length(sparsity)*length(mtrys)*length(supervised)*length(prob))),
                                      nrow = nFolds, ncol = length(sparsity)*length(mtrys)*length(supervised)*length(prob))
  trainTime[[dataSet]][[m]] <- matrix(as.double(rep(NA, nFolds*length(sparsity)*length(mtrys)*length(supervised)*length(prob))),
                                      nrow = nFolds, ncol = length(sparsity)*length(mtrys)*length(supervised)*length(prob))
  OOBTime[[dataSet]][[m]] <- matrix(as.double(rep(NA, nFolds*length(sparsity)*length(mtrys)*length(supervised)*length(prob))),
                                      nrow = nFolds, ncol = length(sparsity)*length(mtrys)*length(supervised)*length(prob))
  testTime[[dataSet]][[m]] <- matrix(as.double(rep(NA, nFolds*length(sparsity)*length(mtrys)*length(supervised)*length(prob))),
                                      nrow = nFolds, ncol = length(sparsity)*length(mtrys)*length(supervised)*length(prob))
  treeStrength[[dataSet]][[m]] <- matrix(as.double(rep(NA, nFolds*length(sparsity)*length(mtrys)*length(supervised)*length(prob))),
                                      nrow = nFolds, ncol = length(sparsity)*length(mtrys)*length(supervised)*length(prob))
  treeCorr[[dataSet]][[m]] <- matrix(as.double(rep(NA, nFolds*length(sparsity)*length(mtrys)*length(supervised)*length(prob))),
                                      nrow = nFolds, ncol = length(sparsity)*length(mtrys)*length(supervised)*length(prob))
  numNodes[[dataSet]][[m]] <- matrix(as.double(rep(NA, nFolds*length(sparsity)*length(mtrys)*length(supervised)*length(prob))),
                                      nrow = nFolds, ncol = length(sparsity)*length(mtrys)*length(supervised)*length(prob))
  ...
  for (k in seq.int(nFolds)) {
    ...
    save(testError, testAUC, OOBError, OOBAUC, trainTime, OOBTime, testTime, treeStrength, treeCorr, numNodes, bestIdx, params, file = paste0(rerfPath, "RandomerForest/R/Results/2018.02.11/", dataSet, "_2018_02_11.RData"))
  }
}