...
rsphere <- function(n, p) {
  ...
  X <- mvrnorm(n, mu, sgma)
  r <- runif(n)^(1/p)
  norm.factor <- sapply(1:n, FUN = function(rw) 2/r[rw]*sqrt(sum(X[rw, ]^2)))
  return(X/norm.factor)
}
...
for (k in 1:length(ns)) {
    ...
    for (trial in 1:num.trials) {
      ...
      # similarity matrix (sim defined as 1 - euclidean distance)
      Q <- diag(1/2, n, n)
      for (j in 1:(n-1)) {
          for (i in (j+1):n) {
              Q[i, j] <- 1 - sqrt(sum((X[i, ] - X[j, ])^2))
          }
      }
      Q <- Q + t(Q)
      # Y <- r
    ...
    save(map1, map10, map200, mae, spearman, train.time, file = '/home/euclidean_similarity.RData')
}
