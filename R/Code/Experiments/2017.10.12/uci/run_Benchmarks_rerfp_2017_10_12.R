...
for (m in classifiers) {
  ...
  testAUC[[dataSet]][[m]] <- matrix(as.double(rep(NA, nFolds*length(sparsity)*length(mtrys))),
                                      nrow = nFolds, ncol = length(sparsity)*length(mtrys))
  ...
  for (k in seq.int(nFolds)) {
    ...
    save(testError, testAUC, OOBError, OOBAUC, trainTime, OOBTime, testTime, treeStrength, treeCorr, numNodes, bestIdx, params, file = paste0(rerfPath, "RandomerForest/R/Results/2017.10.12/", dataSet, "_rerfp_2017_10_12.RData"))
  }
}
