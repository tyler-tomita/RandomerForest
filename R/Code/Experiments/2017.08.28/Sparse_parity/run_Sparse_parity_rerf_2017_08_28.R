...
nTrials <- 10L
classifiers <- c("rf", "rerf", "rerfc", "rerfp", "frc", "frcn", "rr-rf")
testError <- list()
OOBError <- list()
OOBAUC <- list()
trainTime <- list()
...
for (m in classifiers) {
  ...
  for (j in 1:length(ps)) {
    ...
    for (i in 1:length(ns[[j]])) {
      ...
      params[[m]][[(j-1L)*length(ns[[j]]) + i]] <- list(trees = nTrees, randomMatrix = randomMatrix, d = mtrys, sparsity = sparsity, rotate = rotate,
                           rank.transform = rank.transform, MinParent = MinParent, MaxDepth = MaxDepth, NumCores = NumCores, seed = seed)
      ...
      for (trial in 1:nTrials) {
        ...
        # evaluate models
        res <- rerf_eval(Xtrain, Ytrain, Xtest, Ytest, params[[m]][[(j-1L)*length(ns[[j]]) + i]])
        ...
        save(testError, OOBError, OOBAUC, trainTime, OOBTime, testTime, treeStrength, treeCorr, numNodes, bestIdx, params, file = paste(rerfPath, "RerF/Results/2017.08.28/Sparse_parity/Sparse_parity_2017_08_28.RData", sep = ""))
      }
    }
  }
}
