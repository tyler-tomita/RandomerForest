# compare bias and variance on Orthant dataset

rm(list=ls())
options(scipen = 999)

rerfPath <- "~/work/tyler/"
dataPath <- "~/work/tyler/Data/Orthant_bias_variance/"
library(rerf)
...
for (m in classifiers) {
  ...
  # read in test data
  D <- read.table(paste0(dataPath, "Test/Orthant_bias_variance_test.csv"), sep = ",")
  p <- ncol(D) - 1L
  Xtest <- as.matrix(D[, 1:p])
  Ytest <- as.integer(D[, p + 1L])
  ntest <- length(Ytest)
  posteriors <- as.matrix(read.table(paste0(dataPath, "Test/Orthant_bias_variance_test_posteriors.csv"), sep = ","))
  ...
  for (i in 1:length(ns)) {
    ...
    for (trial in 1:nTrials) {
      ...
      D <- read.table(paste0(dataPath, "Train/Orthant_bias_variance_train_n", ntrain, "_trial", trial, ".csv"), sep = ",")
    ...
    save(B, V, SE, VE, BE, testError, OOBError, OOBAUC, trainTime, OOBTime, testTime, treeStrength, treeCorr, numNodes, bestIdx, params,
         file = paste(rerfPath, "RandomerForest/R/Results/2017.12.10/Orthant_bias_variance_2017_12_10_rerf.RData", sep = ""))
  }
}
